...
var default_data_value = {
    min : { value : Number.MAX_VALUE, date : new Date(0)}
};
var default_tag = {
    date : new Date(0),
    type : "stock_price_past_year_low",
    tag_value : { tag : false },
...
if(require.main === module){
    ...
    request(url).then(function(data){
        ...
        Promise.each(container.rawContent.day.filter(function(d){ return new Date(d.date) > start_date;}) , function(content){
            return Promise.resolve(stock_price_past_year(content, res, undefined)).th\en(function(tagres){
                res = tagres;
                console.log(res);
...
var stock_price_past_year = function( content, previousTag, option ){
    ...
    if( pre_data_value.min.date < past_year_date ){
    ...
    var current_data_value = {
        min : { value : pre_data_value.min.value , date : pre_data_value.min.date }
    };

    if(content.close < pre_data_value.min.value){
        tag_value.tag = true;
        current_data_value.min.value = content.close;
        current_data_value.min.date = current_date;
    }
