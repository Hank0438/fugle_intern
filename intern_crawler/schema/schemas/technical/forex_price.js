...
var ForexSchema = new Schema({
    ...
    versionKey: false,
    collection : 'forex_daily'
});

ForexSchema.plugin(updatedPlugin);
ForexSchema.index({symbol_id: 1, year: -1}, {unique: true});
ForexSchema.index({year: -1});
    ...
    get : function(symbolId) {
        ...
        return Promise.resolve(
            ...
            var monthlyData = _.chain(_.cloneDeep(dailyData)).groupBy(function(d) {
                ...
                return moment(d.date).format("YYYYMM");
            }).values().map(function(ticks) {
                return {
                    date:   ticks[ ticks.length-1 ].date,
                    open:   ticks[0].open,
