...
var companyFilter = function(){
    fs.readFileAsync('C:/Users/USER/Desktop/Fugle/company_basic.json').then(function(information){
        return Promise.each((JSON.parse(information)).filter(marketFilter), function(company_basic){
            return Promise.delay(500).then(function(){
                var insertComp = company_basic.symbol_id;
                var insertName = company_basic.name;
...
var marketFilter = function(company_basic){
    return ((company_basic.market_type === "上市" || company_basic.market_type === "上櫃") && company_basic.industry_type !== "存託憑證" 
    && (company_basic.desc).indexOf("下市")===-1 && (company_basic.desc).indexOf("下櫃")===-1 
    && (company_basic.symbol_id).length===4 && company_basic.industry_type === search
    ) 
};
var search = "";
if(process.argv[2]){
